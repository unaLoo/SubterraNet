...
export default class PenerateLayer {
    ...
    set polygon(value) {
        if (value && this.relatedLayer) {
            console.log(this.relatedLayer)
            const { vertexData, indexData } = parse(value, this.relatedLayer.origin)
    ...
    setRelatedLayer(_layer) {
        this.relatedLayer = _layer
        this.relatedLayer.maskTexture = this.maskTexture
    }
    ...
    async onAdd(map, _gl) {
        ...
        this.relatedLayer && (this.relatedLayer.maskTexture = this.maskTexture)
    ...
    /**
       ...
       */
    render(_gl, umatrix) {
        if (!this.relatedLayer) return

        const gl = this.gl
        const Xmatrix = this.relatedLayer.matrix
